...
<GridStackItem bind:this={item} {id} class="max-h-full" width={gridOptions.w} height={gridOptions.h} x={gridOptions.x} y={gridOptions.y}>
  <div class=" h-full w-full">
    <a href={$edit ? undefined : tile.link}>
      <div class="p-4 rounded-box bg-base-200 {$edit && 'transition-colors hover:bg-base-300 cursor-pointer'} { tile.link ? 'hover:bg-base-300 cursor-pointer' : '' } h-full flex flex-col">
        {#if loading}
          <Spinner />
        {:else}
          {#if $edit}
