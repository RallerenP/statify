<script lang="ts">
  ...
  import { createNewMenuItem, getAllMenus } from '../api/api';
  ...
  const handleKeypress = async (e: KeyboardEvent) => {
    ...
    let newItem = await createNewMenuItem({label: value}, '/');
...
{#if items.length || $edit}
  {#each items as item}
    <TreeListItem item={item} depth={0} on:delete={() => refresh()}/>
  {/each}
  {#if $edit}
    <div transition:fade={{duration: 200}} class="h-[48px] my-1 rounded flex">
